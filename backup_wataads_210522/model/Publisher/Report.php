...
class Report extends \System\Db
{
    ...
    public function listActiveWebsite($user_id)
    {
        $stmt = $this->pdo->prepare('SELECT id, domain, status FROM publisher_website WHERE user_id = :user_id AND status = "Active" LIMIT 9999');
    ...
    public function listZone($user_id)
    {
    ...
    {
        $stmt = $this->pdo->prepare('SELECT count(id) AS cnt FROM publisher_report WHERE user_id = :user_id');
        $stmt->bindValue(':user_id', (int) $user_id, \PDO::PARAM_INT);
        $stmt->execute();
    ...
    {
        $stmt = $this->pdo->prepare('SELECT id, name, status, total_sent, created_at, reported_at, date_range FROM publisher_report WHERE user_id = :user_id ORDER BY id DESC LIMIT :init, :limit');
        $stmt->bindValue(':user_id', (int) $user_id, \PDO::PARAM_INT);
    ...
    {
        $stmt = $this->pdo->prepare('SELECT count(id) AS cnt FROM publisher_report WHERE user_id = :user_id AND name LIKE :key');
    ...
    {
        $stmt = $this->pdo->prepare('SELECT id, name, status, total_sent, created_at, reported_at, date_range FROM publisher_report WHERE user_id = :user_id AND name LIKE :key ORDER BY id DESC LIMIT :init, :limit');
        $stmt->bindValue(':key', $key.'%');
        $stmt->bindValue(':user_id', (int) $user_id, \PDO::PARAM_INT);
        $stmt->bindValue(':init', (int) $init, \PDO::PARAM_INT);
        $stmt->bindValue(':limit', (int) $limit, \PDO::PARAM_INT);
    ...
    {
        $stmt = $this->pdo->prepare('INSERT INTO publisher_report (user_id, name, filter, date_range, month_date, week_day, hour) VALUES (:user_id, :name, :filter, :date_range, :month_date, :week_day, :hour)');
        $stmt->bindValue(':user_id', (int) $user_id, \PDO::PARAM_INT);
    ...
    {
        $stmt = $this->pdo->prepare('DELETE FROM publisher_report WHERE id = :id AND user_id = :user_id');
        $stmt->bindValue(':user_id', (int) $user_id, \PDO::PARAM_INT);
        $stmt->bindValue(':id', (int) $id, \PDO::PARAM_INT);
        $stmt->execute();
    ...
    {
        $stmt = $this->pdo->prepare('SELECT * FROM publisher_report WHERE id = :id AND user_id = :user_id');
        $stmt->bindValue(':id', (int) $id, \PDO::PARAM_INT);
        $stmt->bindValue(':user_id', (int) $user_id, \PDO::PARAM_INT);
        $stmt->execute();
    ...
    {
        $stmt = $this->pdo->prepare('SELECT id FROM publisher_report WHERE id = :id AND user_id = :user_id');
        $stmt->bindValue(':id', (int) $id, \PDO::PARAM_INT);
        $stmt->bindValue(':user_id', (int) $user_id, \PDO::PARAM_INT);
        $stmt->execute();
    ...
    {
        $stmt = $this->pdo->prepare('UPDATE publisher_report SET name = :name, filter = :filter, date_range = :date_range, month_date = :month_date, week_day = :week_day, hour = :hour WHERE id = :id');
        $stmt->bindValue(':id', (int) $id, \PDO::PARAM_INT);
    ...
    {
        $stmt = $this->pdo->prepare('SELECT SUM(impression + cpc_impression + cpa_impression) AS impression, SUM(click) AS click, SUM(conversion) AS conversion, SUM(revenue) AS revenue, SUM(reach) AS reach, country FROM publisher_statistic WHERE user_id = :user_id AND created_at  = :created_at GROUP BY country LIMIT 400');
        $stmt->bindValue(':user_id', (int) $user_id, \PDO::PARAM_INT);
