...
class Notification extends \System\Db
{
    ...
    public function listUnreadNotificationByService($user_id, $service, $limit = 10)
    {
        $stmt = $this->pdo->prepare('SELECT id, title, type, service, created_at FROM user_notification WHERE user_id = :user_id AND service = :service AND is_read = 0 ORDER BY id DESC LIMIT :limit');
        $stmt->bindValue(':user_id', (int) $user_id, \PDO::PARAM_INT);
        $stmt->bindValue(':service', $service);
