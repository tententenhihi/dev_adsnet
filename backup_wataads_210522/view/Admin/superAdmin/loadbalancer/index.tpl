<style>
    ...
    .loadbalancer_ram_text, .loadbalancer_cpu_text, .loadbalancer_disk_text, .loadbalancer_net_text, .loadbalancer_bw_text {
        display: flex;
        align-items: center;
        height: 24px;
        margin-top: -4px;
                    ...
                    <div class="dropdown chart-dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownItem3"
                            ...
                            <a class="dropdown-item"
                               href="/loadbalancer/<?=strtolower($status)?><?=$this->arg?>"><?=$status?></a>
                            <?php endforeach; ?>
                ...
                <div class="col-lg-12 col-md-12 col-12">
                    ...
                    <div class="card">
                        <div class="card-content">
                            <div class="card-body card-dashboard">
                                <div class="flatbox">
                                    <div class="level loadbalancer" data-id="<?=$item->id?>" style="flex-wrap: wrap">
                                            ...
                                            <div class="level-item">
                                                <span class="system-meta">
                                                    ...
                                                    <span class="meta-status">
                                                        <progress class="progress-1 is-small is-warning loadbalancer_ram"
                                                                  max="100"></progress>
                                                        <span class="loadbalancer_ram_text">Waiting...</span>
                                                    </span>
                                                </span>
                                            </div>
                                            <div class="level-item">
                                                <span class="system-meta">
                                                    <span>CPU: </span>
                                                    <span class="meta-status">
                                                        <progress class="progress-1 is-small is-warning loadbalancer_cpu"
                                                                  max="100"></progress>
                                                        <span class="loadbalancer_cpu_text">Waiting...</span>
                                                    </span>
                                                </span>
                                            </div>
                                            <div class="level-item">
                                                <span class="system-meta">
                                                    <span>Disk: </span>
                                                    <span class="meta-status">
                                                        <progress class="progress-1 is-small is-warning loadbalancer_disk"
                                                                  max="100"></progress>
                                                        <span class="loadbalancer_disk_text">Waiting...</span>
                                                    </span>
                                                </span>
                                            </div>
                                            <div class="level-item">
                                                <span class="system-meta">
                                                    <span>Net: </span>
                                                    <span class="meta-status">
                                                        <progress class="progress-1 is-small is-warning loadbalancer_net"
                                                                  max="100"></progress>
                                                        <span class="loadbalancer_net_text">Waiting...</span>
                                                    </span>
                                                </span>
                                            </div>
                                            <div class="level-item">
                                                <span class="system-meta">
                                                    <span>BW: </span>
                                                    <span class="meta-status">
                                                        <progress class="progress-1 is-small is-warning loadbalancer_bw"
                                                                  max="100"></progress>
                                                        <span class="loadbalancer_bw_text">Waiting...</span>
                                                    </span>
                                                </span>
                                            </div>
                                        </div>
                            ...
                            <div class="hero">
                                <div class="hero-body">
                                    <div class="has-text-centered">
                                        <p class="text-muted">You do not have any LoadBalancers</p>
                                    </div>
                                </div>
                            </div>
                        </article>
            ...
            <div class="modal-card">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input class="input form-control" id="name" type="text" placeholder="LoadBalancer Name">
                    </div>
                    <div class="form-group">
                        <label class="label">IP Address</label>
                        <input class="input form-control" id="ip_address" type="text" placeholder="LoadBalancer IP" value="">
                    </div>
                    <div class="form-group">
...
<script>
    ...
    function updateStats(data) {
        $.each(data, function (k, v) {
            console.log(v);
            var loadbalancer = $('.loadbalancer[data-id=' + v.id + ']');
            if (loadbalancer.length) {
                loadbalancer.find('.system-status').html(v.connection);

                var ram_percent = Math.round((v.ram_used / v.ram_total) * 100);
                loadbalancer.find('.loadbalancer_ram').attr('value', ram_percent).removeClass('is-warning').addClass(pickClass(ram_percent));
                loadbalancer.find('.loadbalancer_ram_text').html(numberFormat(v.ram_used) + ' / ' + numberFormat(v.ram_total));

                var cpu_percent = v.cpu_user + v.cpu_system + v.cpu_wait;
                loadbalancer.find('.loadbalancer_cpu').attr('value', cpu_percent).removeClass('is-warning').addClass(pickClass(cpu_percent));
                loadbalancer.find('.loadbalancer_cpu_text').html(v.cpu_user + '%us ' + v.cpu_system + '%sy ' + v.cpu_wait + '%wa');

                var disk_percent = Math.round((v.disk_used / v.disk_total) * 100);
                loadbalancer.find('.loadbalancer_disk').attr('value', disk_percent).removeClass('is-warning').addClass(pickClass(disk_percent));
                loadbalancer.find('.loadbalancer_disk_text').html(numberFormat(v.disk_used) + ' / ' + numberFormat(v.disk_total));

                var net_percent = Math.round((v.port_down / v.port_total) * 100);
                loadbalancer.find('.loadbalancer_net').attr('value', net_percent).removeClass('is-warning').addClass(pickClass(net_percent));
                loadbalancer.find('.loadbalancer_net_text').html(up_icon + numberFormat(v.port_up, 0, ['Bps', 'Kbps', 'Mbps', 'Gbps']) + ' - ' + down_icon + numberFormat(v.port_down, 0, ['Bps', 'KBps', 'MBps', 'Gbps']) + ' / ' + numberFormat(v.port_total, 0, ['Bps', 'Kbps', 'Mbps', 'Gbps']));

                var bw_percent = Math.round((v.bandwidth_used / v.bandwidth_total) * 100);
                loadbalancer.find('.loadbalancer_bw').attr('value', bw_percent).removeClass('is-warning').addClass(pickClass(bw_percent));
                loadbalancer.find('.loadbalancer_bw_text').html(numberFormat(v.bandwidth_used) + ' / ' + numberFormat(v.bandwidth_total));
            }
        });
    }

    function fetchStats() {
        $.post('/loadbalancer/stats', {csrf_token: __tajs.csrf}, function (res) {
            if (!res.success) {
    ...
    $(document).ready(function () {
        ...
        $('.loadbalancer').on('click', function () {
        ...
        $('#btn-save').on('click', function () {
            ...
            if (data.ip_address.length < 7) {
                __tajs.showAlert('LoadBalancer IP is required');
                return false;
            }
            $this.addClass('is-loading');
            $this.prop('disabled', true);
            $.post('/loadbalancer', {csrf_token: __tajs.csrf, data: JSON.stringify(data)}, function (res) {
