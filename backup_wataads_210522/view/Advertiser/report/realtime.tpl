    ...
    <div class="row">
        ...
        <div class="col-lg-8 col-sm-12 col-12">
            <div class="card">
                <div class="card-header d-flex">
                    <div>
                        <h4 class="text-bold-700 mb-0">Country <small>(current hour)</small></h4>
                    </div>
                    <div class="dropdown chart-dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-ct" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(92px, 25px, 0px);">
                            ...
                            <a class="dropdown-item select-graph-ct" data-type="cost">By Cost</a>
...
var current_impression = 0, selected_ct = 'impression', selected_rt = 'impression', minute_data = {}, last_data = false;
var country_data = {
    XX: {
        impression: 0,
        click: 0,
...
function updateMinuteData(data) {
    ...
    if(!data) {
        ...
        for (const i in minute_data[current]) {
            var change = data[i];
            if(last_data && last_data[i] <= data[i]) {
...
function realtime() {
    ...
    $.post('/report/realtime', {csrf_token: __tajs.csrf}, function(res) {
        if (res.success) {
            ...
            var impression = last_data && last_data.impression <= data.impression ? data.impression - last_data.impression : data.impression;
