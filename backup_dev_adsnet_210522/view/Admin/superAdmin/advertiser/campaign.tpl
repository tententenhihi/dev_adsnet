...
<div class="content-body">
    ...
    <div class="row">
        <div class="col-12">
            <div class="message text-info" style="background-color: antiquewhite">
                    ...
                    <div class="dropdown chart-dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownItem3"
                            ...
                            <a class="dropdown-item"
                               href="/advertiser/campaign/<?=strtolower($status)?><?=$this->arg?>"><?=$status?></a>
                            <?php endforeach; ?>
    ...
    <div class="row is-multiline" id="results">
                ...
                <div class="card-body card-dashboard">
                    <div class="media">
                        ...
                        <div class="media-content">
                            <h2 class="title is-5" style="font-size: 1.5rem;">
                                <a href="/advertiser/campaign/detail/<?=$item->id?>">#<?=$item->id?> - <span class="item-name"><?=$item->name?></span></a>
                            </h2>
                            <div class="buttons are-transparent" style="margin-top: 5px">
                                ...
                                <a type="button" class="btn btn-default mb-1 waves-effect waves-light" href="/user?id=<?=$item->user_id?>">
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right" id="dropdown-item-<?=$item->id?>" role="menu">
                                            <div class="dropdown-content">
                                                ...
                                                <a class="dropdown-item text-info" href="/advertiser/report?start_date=<?=$item->start_date?>&end_date=<?=$item->end_date?>&campaign_id=<?=$item->id?>&ad_id=&audience_id=&country=&browser=&platform=&os=&group_by=Date&order_by=Date">
                    ...
                    <div class="mt-4">
                        <div class="buttons are-transparent">
                            <a type="button" class="btn btn-success mb-1" href="/advertiser/ad/active?campaign_id=<?=$item->id?>">
                                <i class="feather icon-airplay"></i>
                                <span><?=$item->active_ad?> Active Ads</span>
                            </a>
                            <a type="button" class="btn btn-primary mb-1" href="/advertiser/ad?campaign_id=<?=$item->id?>">
                                <i class="feather icon-airplay"></i>
                            ...
                            <button class="btn btn-danger mb-1">
                                <i class="feather icon-briefcase"></i>
                                <span><?php if(strpos($item->pricing_model, 'cpc') !== false || strpos($item->pricing_model, 'cpa') !== false): ?>e<?php endif; ?>CPM
                                    <?php if($item->impression): ?><?=$this->helper->currencyFormat($item->cost*1000/$item->impression)?><?php else: ?><?=$this->helper->currencyFormat($item->cost)?><?php endif; ?></span>
                            </button>
                            <?php endif; ?>
                            <?php if(strpos($item->pricing_model, 'cpc') !== false): ?>
                            <button class="btn btn-info mb-1">
                                <i class="fa fa-mouse-pointer"></i>
                                <span>CTR <?php if($item->cpc_impression): ?><?=round($item->click/$item->cpc_impression,4)*100?>%<?php else: ?>0.00%<?php endif; ?></span>
                            </button>
                            <button class="btn btn-info mb-1">
                                <i class="feather icon-briefcase"></i>
                                <span>CPC <?php if($item->click): ?><?=$this->helper->currencyFormat($item->cost/$item->click)?><?php else: ?><?=$this->helper->currencyFormat($item->cost)?><?php endif; ?></span>
                            </button>
                            <?php endif; ?>
                            <?php if(strpos($item->pricing_model, 'cpc') !== false): ?>
                            <button class="btn btn-success mb-1">
                                <i class="feather icon-shopping-cart"></i>
                                <span>CR <?php if($item->cpa_impression): ?><?=round($item->conversion/$item->cpa_impression,4)*100?>%<?php else: ?>0.00%<?php endif; ?></span>
                            </button>
                            <button class="btn btn-success mb-1">
                                <i class="fa fa-money"></i>
                                <span>CPA <?php if($item->conversion): ?><?=$this->helper->currencyFormat($item->cost/$item->conversion)?><?php else: ?><?=$this->helper->currencyFormat($item->cost)?><?php endif; ?></span>
                            </button>
                            <?php endif; ?>
                        </div>
                        <hr>
            ...
            <div class="modal-card">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input class="input form-control" id="name" type="text" placeholder="Campaign Name">
                    </div>
                    <div class="form-group">
                        <label class="label">Campaign Description</label>
                        <textarea class="textarea form-control" id="description" placeholder="Some descriptions about this campaign"></textarea>
                    </div>
                </div>
            ...
            if (yes) {
                ...
                $.post('/advertiser/ads', {csrf_token: __tajs.csrf, campaign_id: id, status: ['Active']}, function(res) {
                    ...
                    $.each(res.data, function(k,v){
                        ...
                        html += '<button class="btn btn-warning action-ads" id="'+v.id+'">';
                        ...
                        html += '</button>';
                        html += '<button class="btn btn-info" style="width:170px">'+v.ad_format+'</button>';
                        html += '<button class="btn btn-success">'+v.id+' - '+v.name+'</button>';
                        html += '</div>';
