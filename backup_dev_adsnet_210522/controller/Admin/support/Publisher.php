...
class Publisher extends Controller
{
    ...
    public $order_by = ['Date', 'Impression', 'Click', 'Conversion', 'Revenue', 'Reach', 'Website ID', 'Zone ID'];
    public $report_script = BASE_PATH.'/cli/AdminReporter.php';
    ...
    public function report()
    {
        ...
        $this->data->websites = $db->listActiveWebsite();
    ...
    public function __postReport()
    {
        ...
        $data->order_by = $this->elementInArray($this->getRequest('order_by'), $this->order_by);
        ...
        if($res) {
            ...
            if($res->send_email) $db->sendEmailWhenReportDone($res->id, 0);
            $this->data->data = $this->reportData($data->start_date, $data->end_date, $data->group_by, json_decode($res->data));
            $this->data->id = $res->id;
            $this->renderView();
        }
        ...
        try {
            $this->data->id = $db->createReportTask($user_id, $md5, $filter);
        } catch(\Exception $e) {
            $this->data->data = 'Something was wrong, please try again later!';
            $this->renderView();
        }
        
        // then process report in background
        $this->processReportTaskInBackground($this->data->id);
    ...
    private function reportData($start_date, $end_date, $group_by, $raw)
    {
        $origin = [];
        if($raw) {
            foreach($raw as $item) {
    ...
    private function processReportTaskInBackground($reportId)
    {
        $cmd = $this->report_script.' '.$reportId;
        return shell_exec("nohup $cmd > /dev/null");
    }
}
