...
class System extends Controller
{
    ...
    public function __postCountry()
    {
        ...
        if($action == 'rate') {
            if(!isset($data->format) || !isset($data->country) || !isset($data->admin_ctr) || !isset($data->admin_icr)) {
                $this->data->data = 'Bad request!';
                $this->renderView();
            }
            $data->admin_ctr = $data->admin_ctr ? (float) $data->admin_ctr/100 : null;
            $data->admin_icr = $data->admin_icr ? (float) $data->admin_icr/100 : null;
            if(!$db->updateCountryRate($data->format, $data->country, $data->admin_ctr, $data->admin_icr)) {
    ...
    public function bidding()
    {
        ...
        $this->adServer = new \Module\AdServer;
